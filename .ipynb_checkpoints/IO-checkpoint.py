...
def get_lfp(hf,m,s,period,red):
    ...
    if (m+'/'+s+'/ePhys/lfp' in hf):
        ...
        elif (period == 'Post' and hf[m][s]['traces']['dff'].attrs[period] == 'null'):
            return {'lfp':'NO data for period','FS':None}
        else:
...
def get_rawTrace(hf,m,s,period,red):
    ...
    if m+'/'+s+'/traces/rawTraces' in hf:
        if period == None:
            if red == None:
                return {'f':hf[m][s]['traces']['dff'].value,
                        'FS':1/hf[m][s]['traces']['dff'].attrs['dt'],
                        'numred':int(hf[m][s]['traces']['dff'].attrs['numRed'])}
            else:
                numred = int(hf[m][s]['traces']['dff'].attrs['numRed'])
                print(numred)
                if red:
                    return {'f':hf[m][s]['traces']['rawTraces'][0:numred,:].value,
                            'FS':1/hf[m][s]['traces']['dff'].attrs['dt'],
                            'numred':numred}
                else:
                    return {'f':hf[m][s]['traces']['rawTraces'][numred:,:].value,
                            'FS':1/hf[m][s]['traces']['dff'].attrs['dt'],
                            'numred':numred} 
            ...
            if (period == 'Post' and hf[m][s]['traces']['dff'].attrs[period] == 'null'):
                ...
                numred = hf[m][s]['traces']['dff'].attrs['numRed']
                f = hf[m][s]['traces']['rawTraces'].value
                #dff = dff.T
...
def getSkipList(fileName,m,s):
    # takes in a file name and return all the mice that exsits in file
    with h5py.File(fileName,'r') as hf:
        return hf[m][s]['traces']['skipList'].value
    
def getPdata(fileName,s):
    # takes in a file name and return all the mice that exsits in file
    with h5py.File(fileName,'r') as hf:
        return {'Hi_mvmt_start':hf[s]['hiLo_start'].value,
                'Hi_mvmt_stop':hf[s]['hiLo_end'].value,
                'EPS':hf[s]['EPS'].value,
                'caOnset':hf[s]['caOnset'].value}
